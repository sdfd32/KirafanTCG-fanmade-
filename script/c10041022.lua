...
function s.damcon1(e,tp,eg,ep,ev,re,r,rp)
	...
	and (Duel.GetMatchingGroupCount(Card.IsCode,tp,LOCATION_MZONE,0,nil,10041020)~=0 or Duel.GetMatchingGroupCount(Card.IsCode,tp,LOCATION_MZONE,0,nil,10041021)~=0)
end
...
function s.damcon2(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsBattlePhase() and Duel.GetTurnPlayer()==tp and e:GetHandler():GetCounter(0xd01)>1
	and (Duel.GetMatchingGroupCount(Card.IsCode,tp,LOCATION_MZONE,0,nil,10041020)~=0 or Duel.GetMatchingGroupCount(Card.IsCode,tp,LOCATION_MZONE,0,nil,10041021)~=0)
end
function s.damtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local extraatk=Duel.GetRandomNumber(1,10)
	if (main:IsSetCard(0xd04) and extraatk<=2) or (main:IsSetCard(0xd03) and extraatk<=5)
	or (main:IsSetCard(0xd02) and extraatk<=7) or main:IsSetCard(0xd01) then
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_UPDATE_ATTACK)
	e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_CHAIN)

...
function s.bosstg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.Hint(HINT_MESSAGE,1-tp,aux.Stringid(10041025,8))
end
...
function s.spsummon(e,tp,eg,ep,ev,re,r,rp)
    ...
    fennel=Duel.CreateToken(tp,10041025)
	Duel.SpecialSummon(fennel,0,tp,tp,false,false,POS_FACEUP_ATTACK)
	else
	fennel=Duel.CreateToken(tp,10041026)
	Duel.SpecialSummon(fennel,0,tp,tp,false,false,POS_FACEUP_ATTACK)	end
...
function s.resetop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local a=0
	while a<=20 do
    fennel1=Duel.CreateToken(tp,10041025)
	fennel2=Duel.CreateToken(tp,10041026)

	Duel.SendtoDeck(fennel1,nil,0,REASON_RULE)
	Duel.SendtoDeck(fennel2,nil,0,REASON_RULE)
	a=a+1 end
	Duel.ShuffleDeck(tp)
